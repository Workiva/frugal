...
func (f *FFooClient) BinMethod(ctx *frugal.FContext, bin []byte, str string) (r []byte, err error) {
	...
	if err = f.transport.Register(ctx, f.recvBinMethodHandler(ctx, resultC, errorC)); err != nil {
		return
	}
	defer f.transport.Unregister(ctx)
	f.mu.Lock()
	...
	if err = f.oprot.WriteMessageBegin("bin_method", thrift.CALL, 0); err != nil {
		f.mu.Unlock()
		return
	}
...
func (f *FFooClient) recvBinMethodHandler(ctx *frugal.FContext, resultC chan<- []byte, errorC chan<- error) frugal.FAsyncCallback {
	return func(tr thrift.TTransport) error {
		iprot := f.protocolFactory.GetProtocol(tr)
		if err := iprot.ReadResponseHeader(ctx); err != nil {
			errorC <- err
		...
		if method != "bin_method" {
			err = thrift.NewTApplicationException(thrift.WRONG_METHOD_NAME, "bin_method failed: wrong method name")
			errorC <- err
			return err
		}
		if mTypeId == thrift.EXCEPTION {
		...
		if mTypeId != thrift.REPLY {
			err = thrift.NewTApplicationException(thrift.INVALID_MESSAGE_TYPE_EXCEPTION, "bin_method failed: invalid message type")
			errorC <- err
			return err
		}
...
func NewFFooProcessor(handler FFoo) *FFooProcessor {
	...
	p.AddToProcessorMap("bin_method", &fooFBinMethod{handler: handler, writeMu: p.GetWriteMutex()})
	return p
}
...
func (p *fooFBinMethod) Process(ctx *frugal.FContext, iprot, oprot *frugal.FProtocol) error {
	...
	if err = args.Read(iprot); err != nil {
		iprot.ReadMessageEnd()
		p.writeMu.Lock()
		fooWriteApplicationError(ctx, oprot, thrift.PROTOCOL_ERROR, "bin_method", err.Error())
		p.writeMu.Unlock()
		return err
	}
	...
	var err2 error
	var retval []byte
	if retval, err2 = p.handler.BinMethod(ctx, args.Bin, args.Str); err2 != nil {
		switch v := err2.(type) {
		case *base.APIException:
			result.API = v
		default:
			p.writeMu.Lock()
			fooWriteApplicationError(ctx, oprot, thrift.INTERNAL_ERROR, "bin_method", "Internal error processing bin_method: "+err2.Error())
			p.writeMu.Unlock()
			return err2
		}
	} else {
	...
	if err2 = oprot.WriteResponseHeader(ctx); err2 != nil {
		if err2 == frugal.ErrTooLarge {
			fooWriteApplicationError(ctx, oprot, frugal.RESPONSE_TOO_LARGE, "bin_method", "response too large: "+err2.Error())
			return nil
		}
		err = err2
	}
