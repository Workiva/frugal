...
func NewSecondServiceProcessor(handler SecondService) *SecondServiceProcessor {
	...
	self166 := &SecondServiceProcessor{handler: handler, processorMap: make(map[string]thrift.TProcessorFunction)}
	self166.processorMap["blahBlah"] = &secondServiceProcessorBlahBlah{handler: handler}
	self166.processorMap["secondtestString"] = &secondServiceProcessorSecondtestString{handler: handler}
	return self166
}
...
func (p *SecondServiceProcessor) Process(iprot, oprot thrift.TProtocol) (success bool, err thrift.TException) {
	...
	x167 := thrift.NewTApplicationException(thrift.UNKNOWN_METHOD, "Unknown function "+name)
