...
AlbumWinnersPublisher albumWinnersPublisherFactory(
        frugal.FScopeProvider provider,
...
class AlbumWinnersPublisher {
  late frugal.FPublisherTransport transport;
  late frugal.FProtocolFactory protocolFactory;
  late Map<String, frugal.FMethod> _methods;
  AlbumWinnersPublisher(frugal.FScopeProvider provider,
      [List<frugal.Middleware>? middleware]) {
    ...
    this._methods = {};
    this._methods['ContestStart'] = frugal.FMethod(this._publishContestStart,
        'AlbumWinners', 'publishContestStart', combined);
    this._methods['TimeLeft'] = frugal.FMethod(
        this._publishTimeLeft, 'AlbumWinners', 'publishTimeLeft', combined);
    this._methods['Winner'] = frugal.FMethod(
        this._publishWinner, 'AlbumWinners', 'publishWinner', combined);
  }
  ...
  Future _publishContestStart(frugal.FContext ctx, List<t_v1_music.Album> req) {
    var op = 'ContestStart';
    var prefix = 'v1.music.';
  ...
  Future _publishTimeLeft(frugal.FContext ctx, double req) {
    var op = 'TimeLeft';
    var prefix = 'v1.music.';
    var topic = '${prefix}AlbumWinners$delimiter$op';
    var memoryBuffer = frugal.TMemoryOutputBuffer(transport.publishSizeLimit);
  ...
  Future _publishWinner(frugal.FContext ctx, t_v1_music.Album req) {
    var op = 'Winner';
    var prefix = 'v1.music.';
    var topic = '${prefix}AlbumWinners$delimiter$op';
    var memoryBuffer = frugal.TMemoryOutputBuffer(transport.publishSizeLimit);
...
AlbumWinnersSubscriber albumWinnersSubscriberFactory(
        frugal.FScopeProvider provider,
        {List<frugal.Middleware>? middleware}) =>
...
class AlbumWinnersSubscriber {
  ...
  Future<frugal.FSubscription> subscribeContestStart(
      dynamic onlist(frugal.FContext ctx, List<t_v1_music.Album> req)) async {
    var op = 'ContestStart';
    var prefix = 'v1.music.';
    var topic = '${prefix}AlbumWinners$delimiter$op';
    var transport = provider.subscriberTransportFactory.getTransport();
    await transport.subscribe(
        topic, _recvContestStart(op, provider.protocolFactory, onlist));
    return frugal.FSubscription(topic, transport);
  }

  frugal.FAsyncCallback _recvContestStart(
      String op,
      frugal.FProtocolFactory protocolFactory,
      dynamic onlist(frugal.FContext ctx, List<t_v1_music.Album> req)) {
    frugal.FMethod method = frugal.FMethod(
        onlist, 'AlbumWinners', 'subscribelist', this._middleware);
    callbackContestStart(thrift.TTransport transport) {
      ...
      thrift.TMessage tMsg = iprot.readMessageBegin();
      if (tMsg.name != op) {
      ...
      List<t_v1_music.Album> req = <t_v1_music.Album>[];
  ...
  Future<frugal.FSubscription> subscribeTimeLeft(
      dynamic onMinutes(frugal.FContext ctx, double req)) async {
    var op = 'TimeLeft';
    var prefix = 'v1.music.';
    var topic = '${prefix}AlbumWinners$delimiter$op';
    var transport = provider.subscriberTransportFactory.getTransport();
    await transport.subscribe(
        topic, _recvTimeLeft(op, provider.protocolFactory, onMinutes));
    return frugal.FSubscription(topic, transport);
  }

  frugal.FAsyncCallback _recvTimeLeft(
      String op,
      frugal.FProtocolFactory protocolFactory,
      dynamic onMinutes(frugal.FContext ctx, double req)) {
    frugal.FMethod method = frugal.FMethod(
        onMinutes, 'AlbumWinners', 'subscribeMinutes', this._middleware);
  ...
  Future<frugal.FSubscription> subscribeWinner(
      dynamic onAlbum(frugal.FContext ctx, t_v1_music.Album req)) async {
    var op = 'Winner';
    var prefix = 'v1.music.';
    var topic = '${prefix}AlbumWinners$delimiter$op';
    var transport = provider.subscriberTransportFactory.getTransport();
    await transport.subscribe(
        topic, _recvWinner(op, provider.protocolFactory, onAlbum));
    return frugal.FSubscription(topic, transport);
  }

  frugal.FAsyncCallback _recvWinner(
      String op,
      frugal.FProtocolFactory protocolFactory,
      dynamic onAlbum(frugal.FContext ctx, t_v1_music.Album req)) {
    frugal.FMethod method = frugal.FMethod(
        onAlbum, 'AlbumWinners', 'subscribeAlbum', this._middleware);
