...
var Languages = LanguageOptions{
	"go": Options{
		...
		"filter_yaml":                 "If specified, it will check the filename given (default: frugal_filter.yaml) for filter instructions",
		"debugFilter":                 "If this is included as an option, it will print debug output of the filter portion of generation.",
	},
	"java": Options{
...
type filterInputer interface {
	FilterInput(*parser.Frugal)
}
...
func (o *programGenerator) Generate(frugal *parser.Frugal, outputDir string) error {
	if fi, ok := o.LanguageGenerator.(filterInputer); ok {
		fi.FilterInput(frugal)
	}
